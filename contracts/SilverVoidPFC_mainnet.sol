...
/**
 * @title SilverVoidPFC (mainnet revision)
 ...
 *
 * 1. PAGINATED GETTER — getDuelsBatch(fromId, count). The frontend used to
 *    probe getDuel(id) one call at a time for every duel id (the duelist
 *    leaderboard alone was up to 2000 sequential RPC round-trips per scan).
 *    One call now returns up to 100 duels.
 *
 ...
 *    instead of scanning event logs over block ranges (the old approach
 *    needed a full-history eth_getLogs scan per wallet, one of the heaviest
 *    requests an RPC node can serve).
 *
 ...
 *    claimable anytime via withdraw(). Rationale: transfers use a 30k gas
 *    stipend (deliberate, anti-reentrancy), but smart-contract wallets
 *    (Safe, AA accounts) can need more than that in receive() — under the
 *    old code a winner using such a wallet could NEVER be paid: reveal()
 *    reverted every time, permanently. Burns to the dead address keep
 *    their original behavior (dead address has no code; cannot fail).
 *
 * The commit scheme (keccak256(abi.encodePacked(uint8 choice, bytes32
 * secret))), stake tiers, reveal window, split percentages, and all event
 * signatures are UNCHANGED — the existing frontend logic works as-is.
 */
contract SilverVoidPFC {
    ...
    // ═══ Player index (new) — duel ids per wallet, filled at create/join ═══
    ...
    ///         counts RESOLVED duels — a subtlety that broke the frontend's
    ///         scan bounds on testnet), this covers every duel including
    ...
    ///         replaces the frontend's old getDuel()-per-id probing (up to
    ///         2000 sequential calls per leaderboard scan).
    ...
    function _distribute(uint256 total, address winner) private {
        ...
        if (burned) {
            ...
            // as a defensive fallback mirroring the original contract:
            // route to CREATOR (deferred if needed) rather than lock funds.
    ...
    ///      for later withdraw() instead of reverting the whole tx. This is
    ///      what makes reveal() and claimRevealTimeout() safe for winners
    ///      using Safe/AA wallets.
