 ...
 *
 *      CHANGE FROM ORIGINAL: _imageURI() now returns "ipfs://<CID>" instead
 *      of a hardcoded gateway URL (https://beige-defiant-boa-301.mypinata.cloud/ipfs/<CID>).
 *      Wallets and marketplaces (MetaMask, OpenSea, Rarible, etc.) resolve
 *      ipfs:// URIs themselves, usually with their own gateway + fallback
 *      infrastructure — so which specific gateway is up or down becomes
 *      each client's problem to solve, not something baked immutably into
 *      every already-minted token. This is the standard, recommended
 *      practice for NFT image URIs.
 */
...
contract SilverVoidNFT {
    ...
    // CHANGED (v2): images now live on Arweave — pay-once permanent storage.
    // First revision replaced the hardcoded Pinata gateway with ipfs://,
    // which fixed the single-gateway dependency but still relied on someone
    // (a pinning service subscription) keeping the content alive on IPFS.
    // Arweave removes that last dependency: the protocol itself guarantees
    // permanence, nothing to maintain or renew, ever.
    function _imageURI(uint8 rank, uint8 nftId) private pure returns (string memory) {
        string memory base = "https://arweave.net/";
        if (rank == 1 && nftId == 0) return string(abi.encodePacked(base, "EHnPIGelwmnj2Tkue5b3QmLTqv8jSgpi0U1CiCsdK1o"));
        if (rank == 1 && nftId == 1) return string(abi.encodePacked(base, "4x2F1xk7jjg9ITb1GxrFbhgz6oo6YpsL2wOnBJ-zLHQ"));
        if (rank == 1 && nftId == 2) return string(abi.encodePacked(base, "BiySJvr78rNGykb_sP1-yZyw4G4nzWFJSpU4FqocYZ4"));
        if (rank == 2 && nftId == 0) return string(abi.encodePacked(base, "uTmPDOzrbfT-uUyO0Qhb-ilVHfy2h5twNQHs9mvzKTU"));
        if (rank == 2 && nftId == 1) return string(abi.encodePacked(base, "EWd71s8uhAKqopRbAXqCE5aQ0QIndhB3AM_ipyV_B0s"));
        if (rank == 2 && nftId == 2) return string(abi.encodePacked(base, "UiWkV9G0thjQcPv_5WAZec3VteaLvVPAbTZppQWL6V8"));
        if (rank == 3 && nftId == 0) return string(abi.encodePacked(base, "SKkYwXGMRSj9PLn-yU-n9bPMPEW8WCDO8K-WCnuE3v0"));
        if (rank == 3 && nftId == 1) return string(abi.encodePacked(base, "NAhGa6acRRxbhC09zgtMQ_5s8HpM-HOoAFzfsSJVTk4"));
        if (rank == 4 && nftId == 0) return string(abi.encodePacked(base, "6Hj22YuEde4dNm8LxOQg9yaOSKYU_U98LbBUMEiSQyg"));
        return "";
    }
